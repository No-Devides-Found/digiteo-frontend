...
const BackBox = styled(Box)({
  ...
  top: "297px",
  background:
    "linear-gradient(180deg, #FFFDE7 0%, rgba(217, 217, 217, 0) 100%)",
});
...
function PracticeList() {
  ...
  const getPracticeList = async () => {
    try {
      // const resp = (await axios.get("/posts/practice-list")).data; ////localhost:3000/practicelist
      const resp = await axios.get("/posts/practice/"); // 임시로 검색어상관없이 모든 창작게시물 불러오는 코드
      ...
      <Grid container style={{ padding: "5rem 13rem" }}>
        ...
        <Grid item xs={12} mt={5}>
          <BrownGrid container style={{ justifyContent: "center" }}>
            <GreenGrid container>
              <ListTemplate practices={practiceList} page={page} />
            </GreenGrid>
          </BrownGrid>
        </Grid>
      </Grid>
